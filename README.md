...
решение вопроса минимизации и ускорения скетчей созданных с использованием Wiring. Дальнейшее развитие привело к
созданию "автоматной части" библиотеки, позволяющей устранить delay() практически полностью.

  С развитием, библиотека стала претендовать на замену Wiring полностью .. этакий Wiring++ при сохранении возможности
такого же простого вхождения в программирование Ардуино, но с добавлением способности создавать максимально компактные
И шустрые результаты, не существенно хуже чем профессиональная разработка на ассемблере.
  ...
  * `#define pinForShield  13` -- *так правильно*
  ...
  Пожелания по разработке - принимаются, но их реализация только "по мере наличия времени".

Возвращены "режимы" компиляции библиотеки предопределением константы ARHAT_MODE до подключения библиотеки:
* `#define ARHAT_MODE 1` -- "совмещенный режим". Подключается типовой Wiring, все типовые функции берутся из него вместе
с обработчиком прерываний таймера0 (все временные функции). Соответственно "минимизация и ускорение" кода достижимо
 только при использовании макросов и функций либы по их прямым именам. Типовые имена digitalRead() и пр. - вызываются из Wiring.
* `#define ARHAT_MODE 2` и если константа не определена вовсе -- "автономный режим". Wiring *не* подключается, все типовые функции
из него переопределяются на макросы библиотеки.

### Что есть и как работает (принципы построения либы)
...
Этим и пользуемся: Файл arhat.h в автономном режиме имитирует "как будто бы" этот файл (Arduino.h), УЖЕ был запущен и подключен.
...
А также сделано консистентное чтение счетчика переполнений getOvfCount() и millis() как таковая стала не востребована.
...
с укороченными типами передаваемых параметров: uint8_t и uint16_t вместо uint32_t или long. Часть функций Wiring
уже переименована именно в такие версии. В частности delay().

*4. Сохранена возможность переопределения деталей реализации конкретной машинки и её микроконтроллера.

Вся реализация, в настоящее время выполнена и тестируется под машинку Arduino Mega2560 USB. Тем не менее,
...
*analogRead() // функция последовательного чтения аналога. Ожидает завершения операции, использует текущий режим настроек.
...
*everyOvf()    -- делает тоже самое, но на основе счетчика переполнений таймера и соответственно работает в "тиках" по 1024 микросекунды
...
*и эти функции изменены:
*millis() -> time_millis()
*micros() -> time_micros()
*delay()  -> time_delay16(ms) -- использует 16-и битный параметр! Сокращает вызовы на 6 байт каждый, но считает только до 65.5секунд..

*Примечание:* возвращен выбор режимов компиляции скетчей константой ARHAT_MODE. В режиме совместимости все типовые имена функций
...
### Внимание!
  Если у Вас при запуске скетчей из примеров появляется ошибка типа 'redefine __vector_xx()' (xx = 23 для Мега2560 и =16 для УНО 328P),
...
ПРИ ЗАПУСКЕ распакованной ИДЕ БЕЗ какой либо инсталляции, в т.ч. и предварительной и других версий.
...
особенно специальным и, соответственно, всегда явно известны при разработке скетчей.
  Желание "сделать файл настроек" или "а я не знаю какой будет пин" - тут избыточно и имеет больше вреда, чем пользы.
