...
* Макросы примитивов работы с PC_INT
...
`#define MAX_PULSES 4<br/>#define PCINT 2<br/>#include "pcint.h"<br/>#define PCINT 1<br/>#include "pcint.h"`
`<br/>Обрабатываем 4 вектора в двух уровнях прерываний. Какие и сколько в функциях ниже`

##### вспомогательные макросы для получения регистров, бит и текстовок (ресурсов МК) по уровню прерывания: [0,1,2]
...
##### Константы, определенные для обработки прерываний PCINT

  * PULSE_BUSY, PULSE_SECOND, PULSE_OK, PULSE_TIMER, PULSE_ERROR -- состояния обработчика прерываний.
  * PULSE_RAISING,PULSE_FAILING,PULSE_BOTH -- режимы подсчета количества прерываний (энкодер)

  * PCINT -- номер уровня прерывания включения файла pcint.h
  * MAX_PULSES -- сколько всего (всех уровней) будет обрабатываться прерываний - указывается 1 раз перед первым включением.

##### Типы данных и структуры для PCINT
...
##### Структуры харанения данных для обработчика прерывания заданного уровня:

`Структуры определяются и добавляются в код только для указанного уровня прерываний PCINT!`
...
##### Функции работы с прерываниями PCINT
...
Поскольку все функции авто-создаются через подключение файла pcint.h с предварительно установленным номером уровня прерывания,
 постольку в названиях функций добавляет номер уровня, ниже обозначенный как [0..2]. Для использования конкретной функции,
 в её имени требуется указывать номер уровня прерывания, например: pcint**0**_init(параметры) 

 Инициализация обработчика прерывания требуемого уровня. Возвращает номер вектора прерывания [0..7]:
  * uint8_t  **pcint[0..2]_init**(uint8_t pulseID, uint8_t pin, uint8_t state, PcintMethod method, TSC_Time timeout);

 Запуск замера по вектору [0..7] заданного уровня прерываний. Номер вектора возвращает функция pcint_init() (см. выше)
  * void     **pcint[0..2]_start**(uint8_t intNumber);

 Измерение длительности импульса по прерыванию с ожиданием результата замера (аналог pulseIn)
  * TSC_Time **pulseIn[0..2]**(uint8_t pulseId, void (*action)(void));
...
##### Методы работы обработчиков PCINT (пока только 2)
